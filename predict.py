...
class Model:
    ...
    def __init__(
        ...
        data_adapter_fn,
        use_kv_cache: Optional[Literal["none",
                                       "flash_decoding", "vanilla"]] = None,
    ):
    ...
    def _init_model(self):
        ...
        if self.use_kv_cache is not None:
            if "causal" in self.checkpoint_config and self.checkpoint_config["causal"] is False:
                raise Exception(
                    "kv_cache not supported for non-causal models!")
    ...
    def causal_sample(
        ...
        *,
        ...
        for i, encoded_text in enumerate(encoded_texts):
            ...
            xs.append(
                torch.cat(
                    ...
                    [encoded_text, * \
                        [torch.ones_like(encoded_text) * 1024] * (len(self.vocab_sizes) - 1)],
                    dim=0,
                ).unsqueeze(0)
            )  # b x [(b=1, c, t)]
        ...
        x = torch.zeros(
            (len(encoded_texts), xs[0].shape[1], max_len), dtype=torch.long, device=self.config.device)
            ...
            with self._ctx:  # type: ignore
                ...
                for k in range(self.config.num_samples):
                    ...
                    if max(seq_lens) + max_new_tokens >= self.model.config.block_size:
                        ...
                        '''
                        max_new_tokens = self.model.config.block_size - max(seq_lens) - 1
                    ...
                    for i in range(len(y)):
                        to_return.append(self.decoder.decode(
                            tokens=y[i].tolist(), causal=True))
    ...
    def non_causal_sample(
        ...
        *,
        ...
        for encoded_text, encodec_token in zip(encoded_texts, encodec_tokens):
            ...
            assert len(encodec_token) >= 1 and len(
                encodec_token) <= self._num_encodec_codebooks
            ...
            hierarchies_in.append(
                ...
                len(text_tokens) +
                encodec_token[1] + [self._encodec_codes_pad_token]
            )
            ...
            for _, t_hierarchy in enumerate(hierarchies_in):
                ...
                if len(t_hierarchy) < self._encodec_ctx_window:
                    padded_hierarchies_input.append(
                        t_hierarchy + [self._encodec_codes_pad_token] *
        ...
        # check that the input is correct
        in_x = torch.tensor(padded_hierarchies_inputs,
...
def get_cached_file(file_or_uri: str):
    ...
    if is_uri:
        ...
        _cache_name = "audio_" + \
            hashlib.md5(file_or_uri.encode("utf-8")).hexdigest() + ext
...
def get_cached_embedding(local_file_path: str, spkemb_model):
    ...
    # hash the file path to get the cache name
    _cache_name = "embedding_" + \
        hashlib.md5(local_file_path.encode("utf-8")).hexdigest() + ".pt"
    ...
    if not os.path.exists(cache_path):
        spk_emb = spkemb_model.embed_utterance_from_file(
            local_file_path, numpy=False).unsqueeze(0)  # (b=1, c)
...
def _sample_utterance_batch(
    ...
    for i, (text, spk_cond_path) in tqdm.tqdm(
        ...
        speaker_embs.append(get_cached_embedding(
            spk_cond_path, spkemb_model) if spk_cond_path else None)
    ...
    for text, tokens, speaker_embs, ref_name, wav_file in zip(texts, b_tokens, b_speaker_embs, refs, wav_files):
        ...
        with tempfile.NamedTemporaryFile(suffix=".wav") as enhanced_tmp:
            if enhancer is not None:
                enhancer = get_enhancer(enhancer) if isinstance(
                    enhancer, str) else enhancer
...
def build_models(config_first_stage, config_second_stage, device, use_kv_cache):
    ...
    data_adapter = FlattenedInterleavedEncodec2Codebook(
